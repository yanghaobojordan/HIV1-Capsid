...
def main():
    ...
    mutate_residue(pose, pose.pdb_info().pdb2pose('O', 1417), "G")
    mutate_residue(pose, pose.pdb_info().pdb2pose('P', 1417), "G")
    mutate_residue(pose, pose.pdb_info().pdb2pose('M', 1417), "G")
    mutate_residue(pose, pose.pdb_info().pdb2pose('T', 1417), "G")
    mutate_residue(pose, pose.pdb_info().pdb2pose('U', 1417), "G")
    mutate_residue(pose, pose.pdb_info().pdb2pose('V', 1417), "G")
...
def MC(pose, scorefxn, mutant):
    test=Pose()
    test.assign(pose)
    dumpfile = 'Folding_Output_F1417'+str(mutant)+'.pdb'
    txtfile = 'Folding_Output_F1417'+str(mutant)+'.txt'
    moveList= 'Folding_Output_F1417'+str(mutant)+'_MoveList.txt'
    ...
    residue=int(test.pdb_info().pdb2pose('O', 1417))
    ...
    for i in range(1, test.total_residue()+1):
        ...
        if (residue-i_residue).norm()<10:
            move_list.append(i)
            count +=1
    residue=int(test.pdb_info().pdb2pose('P', 1417))
    residue=test.residue(residue).xyz("CA")
    for i in range(1, test.total_residue()+1):
        i_residue=test.residue(i).xyz("CA")
        if (residue-i_residue).norm()<10:
            move_list.append(i)
            count +=1
    residue=int(test.pdb_info().pdb2pose('M', 1417))
    residue=test.residue(residue).xyz("CA")
    for i in range(1, test.total_residue()+1):
        i_residue=test.residue(i).xyz("CA")
        if (residue-i_residue).norm()<10:
            move_list.append(i)
            count +=1
    residue=int(test.pdb_info().pdb2pose('T', 1417))
    residue=test.residue(residue).xyz("CA")
    for i in range(1, test.total_residue()+1):
        i_residue=test.residue(i).xyz("CA")
        if (residue-i_residue).norm()<10:
            move_list.append(i)
            count +=1
    residue=int(test.pdb_info().pdb2pose('U', 1417))
    residue=test.residue(residue).xyz("CA")
    for i in range(1, test.total_residue()+1):
        i_residue=test.residue(i).xyz("CA")
        if (residue-i_residue).norm()<10:
            move_list.append(i)
            count +=1
    residue=int(test.pdb_info().pdb2pose('V', 1417))
    residue=test.residue(residue).xyz("CA")
    for i in range(1, test.total_residue()+1):
        i_residue=test.residue(i).xyz("CA")
        if (residue-i_residue).norm()<10:
    ...
    smallmover=SmallMover(mm, kT, 1) #3 is the number of moves
    #smallmover.angle_max(7)
    shearmover=ShearMover(mm, kT, 1) #5 is the number of moves
    ...
    mc.boltzmann(test)
    mc.recover_low(test)
    newfile.write('Minimization Compelte')
    newfile.write('    ')
    newfile.write(str(scorefxn(test)))
    newfile.write('\n')
