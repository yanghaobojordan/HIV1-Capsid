...
def main():
    ...
    mutate_residue(pose, pose.pdb_info().pdb2pose('O', 1413), "W")
    mutate_residue(pose, pose.pdb_info().pdb2pose('P', 1413), "W")
    mutate_residue(pose, pose.pdb_info().pdb2pose('M', 1413), "W")
    mutate_residue(pose, pose.pdb_info().pdb2pose('T', 1413), "W")
    mutate_residue(pose, pose.pdb_info().pdb2pose('U', 1413), "W")
    mutate_residue(pose, pose.pdb_info().pdb2pose('V', 1413), "W")
...
def MC(pose, scorefxn, mutant):
    ...
    residue=int(test.pdb_info().pdb2pose('O', 1413))
    ...
    for i in range(1, test.total_residue()+1):
        ...
        if (residue-i_residue).norm()<10:
            move_list.append(i)
            count +=1
    residue=int(test.pdb_info().pdb2pose('P', 1413))
    residue=test.residue(residue).xyz("CA")
    for i in range(1, test.total_residue()+1):
        i_residue=test.residue(i).xyz("CA")
        if (residue-i_residue).norm()<10:
            move_list.append(i)
            count +=1
    residue=int(test.pdb_info().pdb2pose('M', 1413))
    residue=test.residue(residue).xyz("CA")
    for i in range(1, test.total_residue()+1):
        i_residue=test.residue(i).xyz("CA")
        if (residue-i_residue).norm()<10:
    ...
    for i in range(1, test.total_residue()+1):
        ...
        if (residue-i_residue).norm()<10:
            move_list.append(i)
            count +=1
    residue=int(test.pdb_info().pdb2pose('U', 1413))
    residue=test.residue(residue).xyz("CA")
    for i in range(1, test.total_residue()+1):
        i_residue=test.residue(i).xyz("CA")
        if (residue-i_residue).norm()<10:
