...
def main():
    ...
    mutate_residue(pose, pose.pdb_info().pdb2pose('O', 1411), "M")
    mutate_residue(pose, pose.pdb_info().pdb2pose('P', 1411), "M")
    mutate_residue(pose, pose.pdb_info().pdb2pose('M', 1411), "M")
    mutate_residue(pose, pose.pdb_info().pdb2pose('T', 1411), "M")
    mutate_residue(pose, pose.pdb_info().pdb2pose('U', 1411), "M")
    mutate_residue(pose, pose.pdb_info().pdb2pose('V', 1411), "M")
...
def MC(pose, scorefxn, mutant):
    test=Pose()
    test.assign(pose)
    dumpfile = 'Folding_Output_P1411'+str(mutant)+'.pdb'
    txtfile = 'Folding_Output_P1411'+str(mutant)+'.txt'
    moveList= 'Folding_Output_P1411'+str(mutant)+'_MoveList.txt'
    ...
    residue=int(test.pdb_info().pdb2pose('O', 1411))
    ...
    for i in range(1, test.total_residue()+1):
        ...
        if (residue-i_residue).norm()<10:
            move_list.append(i)
            count +=1
    residue=int(test.pdb_info().pdb2pose('P', 1411))
    residue=test.residue(residue).xyz("CA")
    for i in range(1, test.total_residue()+1):
        i_residue=test.residue(i).xyz("CA")
        if (residue-i_residue).norm()<10:
            move_list.append(i)
            count +=1
    residue=int(test.pdb_info().pdb2pose('M', 1411))
    residue=test.residue(residue).xyz("CA")
    for i in range(1, test.total_residue()+1):
        i_residue=test.residue(i).xyz("CA")
        if (residue-i_residue).norm()<10:
            move_list.append(i)
            count +=1
    residue=int(test.pdb_info().pdb2pose('T', 1411))
    residue=test.residue(residue).xyz("CA")
    for i in range(1, test.total_residue()+1):
        i_residue=test.residue(i).xyz("CA")
        if (residue-i_residue).norm()<10:
            move_list.append(i)
            count +=1
    residue=int(test.pdb_info().pdb2pose('U', 1411))
    residue=test.residue(residue).xyz("CA")
    for i in range(1, test.total_residue()+1):
        i_residue=test.residue(i).xyz("CA")
        if (residue-i_residue).norm()<10:
            move_list.append(i)
            count +=1
    residue=int(test.pdb_info().pdb2pose('V', 1411))
    residue=test.residue(residue).xyz("CA")
    for i in range(1, test.total_residue()+1):
        i_residue=test.residue(i).xyz("CA")
        if (residue-i_residue).norm()<10:
