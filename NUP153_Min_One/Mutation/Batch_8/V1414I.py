...
def main():
    ...
    mutate_residue(pose, pose.pdb_info().pdb2pose('O', 1414), "I")
    mutate_residue(pose, pose.pdb_info().pdb2pose('P', 1414), "I")
    mutate_residue(pose, pose.pdb_info().pdb2pose('M', 1414), "I")
    mutate_residue(pose, pose.pdb_info().pdb2pose('T', 1414), "I")
    mutate_residue(pose, pose.pdb_info().pdb2pose('U', 1414), "I")
    mutate_residue(pose, pose.pdb_info().pdb2pose('V', 1414), "I")
...
def MC(pose, scorefxn, mutant):
    test=Pose()
    test.assign(pose)
    dumpfile = 'Folding_Output_V1414'+str(mutant)+'.pdb'
    txtfile = 'Folding_Output_V1414'+str(mutant)+'.txt'
    moveList= 'Folding_Output_V1414'+str(mutant)+'_MoveList.txt'
    ...
    residue=int(test.pdb_info().pdb2pose('O', 1414))
    ...
    for i in range(1, test.total_residue()+1):
        ...
        if (residue-i_residue).norm()<10:
            move_list.append(i)
            count +=1
    residue=int(test.pdb_info().pdb2pose('P', 1414))
    residue=test.residue(residue).xyz("CA")
    for i in range(1, test.total_residue()+1):
        i_residue=test.residue(i).xyz("CA")
        if (residue-i_residue).norm()<10:
            move_list.append(i)
            count +=1
    residue=int(test.pdb_info().pdb2pose('M', 1414))
    residue=test.residue(residue).xyz("CA")
    for i in range(1, test.total_residue()+1):
        i_residue=test.residue(i).xyz("CA")
        if (residue-i_residue).norm()<10:
            move_list.append(i)
            count +=1
    residue=int(test.pdb_info().pdb2pose('T', 1414))
    residue=test.residue(residue).xyz("CA")
    for i in range(1, test.total_residue()+1):
        i_residue=test.residue(i).xyz("CA")
        if (residue-i_residue).norm()<10:
            move_list.append(i)
            count +=1
    residue=int(test.pdb_info().pdb2pose('U', 1414))
    residue=test.residue(residue).xyz("CA")
    for i in range(1, test.total_residue()+1):
        i_residue=test.residue(i).xyz("CA")
        if (residue-i_residue).norm()<10:
            move_list.append(i)
            count +=1
    residue=int(test.pdb_info().pdb2pose('V', 1414))
    residue=test.residue(residue).xyz("CA")
    for i in range(1, test.total_residue()+1):
        i_residue=test.residue(i).xyz("CA")
        if (residue-i_residue).norm()<10:
