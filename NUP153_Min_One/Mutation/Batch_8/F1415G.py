...
def main():
    ...
    mutate_residue(pose, pose.pdb_info().pdb2pose('O', 1415), "G")
    mutate_residue(pose, pose.pdb_info().pdb2pose('P', 1415), "G")
    mutate_residue(pose, pose.pdb_info().pdb2pose('M', 1415), "G")
    mutate_residue(pose, pose.pdb_info().pdb2pose('T', 1415), "G")
    mutate_residue(pose, pose.pdb_info().pdb2pose('U', 1415), "G")
    mutate_residue(pose, pose.pdb_info().pdb2pose('V', 1415), "G")
...
def MC(pose, scorefxn, mutant):
    test=Pose()
    test.assign(pose)
    dumpfile = 'Folding_Output_F1415'+str(mutant)+'.pdb'
    txtfile = 'Folding_Output_F1415'+str(mutant)+'.txt'
    moveList= 'Folding_Output_F1415'+str(mutant)+'_MoveList.txt'
    ...
    residue=int(test.pdb_info().pdb2pose('O', 1415))
    ...
    for i in range(1, test.total_residue()+1):
        ...
        if (residue-i_residue).norm()<10:
            move_list.append(i)
            count +=1
    residue=int(test.pdb_info().pdb2pose('P', 1415))
    residue=test.residue(residue).xyz("CA")
    for i in range(1, test.total_residue()+1):
        i_residue=test.residue(i).xyz("CA")
        if (residue-i_residue).norm()<10:
            move_list.append(i)
            count +=1
    residue=int(test.pdb_info().pdb2pose('M', 1415))
    residue=test.residue(residue).xyz("CA")
    for i in range(1, test.total_residue()+1):
        i_residue=test.residue(i).xyz("CA")
        if (residue-i_residue).norm()<10:
            move_list.append(i)
            count +=1
    residue=int(test.pdb_info().pdb2pose('T', 1415))
    residue=test.residue(residue).xyz("CA")
    for i in range(1, test.total_residue()+1):
        i_residue=test.residue(i).xyz("CA")
        if (residue-i_residue).norm()<10:
            move_list.append(i)
            count +=1
    residue=int(test.pdb_info().pdb2pose('U', 1415))
    residue=test.residue(residue).xyz("CA")
    for i in range(1, test.total_residue()+1):
        i_residue=test.residue(i).xyz("CA")
        if (residue-i_residue).norm()<10:
            move_list.append(i)
            count +=1
    residue=int(test.pdb_info().pdb2pose('V', 1415))
    residue=test.residue(residue).xyz("CA")
    for i in range(1, test.total_residue()+1):
        i_residue=test.residue(i).xyz("CA")
        if (residue-i_residue).norm()<10:
